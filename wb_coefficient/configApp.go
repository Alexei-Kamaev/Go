...
func checkConfigAppRedis() error {
	...
	if data, err := getStringValueRedis(key); err != nil {
		return err
	} else if len(data) == 0 || checkOldConfig() {
...
func checkOldConfig() bool {

	info, err := os.Stat(configFile)
	if err != nil {
		addLog("ошибка при проверке возраста config.json: %v", err)
		return false
	}
	...
	if age < 2*time.Minute {
		addLog("конфигурационный файл %s был обновлён в Redis", configFile)
		return true
	} else {
		return false
	}
